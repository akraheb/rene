...
imagettftext( $my_img, $sizeN, 0, $orez*2, $orez*2, $line_colour2 ,$font, $kontr."/".$count14."-".$vyber." ".$pos." ".$dulux[$q][3]);
...
if(strlen($dulux[$p][0])>$dlhsie)
    ...
    //1    ODDELENE POMLCKOU
        ...
        {
        if($kont<1)
        imagettftext( $my_img, $sizeN, 0, $odsadX+$sizeX*$i+$left-$zelena, $odsadY+$sizeY*$j+$sizeN+$up, $text_colour,$fontL, "1".substr($dulux[$p][0],0,stripos($dulux[$p][0]," – ")));
        else
    ...
    //    2   
        {
        $medzerapo=45;
        $poziciadvoj=stripos($dulux[$p][0],"  ");
        if ($poziciadvoj>$medzerapo)
            $dlhsie2=50;
        else
            $dlhsie2=50;
        $poziciamedzery=stripos($dulux[$p][0]," ",$dlmedzerapo);
 ...
 //       $dlhsie2=53;
        if($kont<1)
        imagettftext( $my_img, $sizeN, 0, $odsadX+$sizeX*$i+$left-$zelena, $odsadY+$sizeY*$j+$sizeN+$up, $text_colour,$fontL, "2".substr($dulux[$p][0],0,$dlhsie2));
        else
    ...
    {
    if($kont<1)
    imagettftext( $my_img, $sizeN, 0, $odsadX+$sizeX*$i+$left-$zelena, $odsadY+$sizeY*$j+$sizeN+$up, $text_colour,$fontL, "0".substr($dulux[$p][0],0,$dlhsie));
    else
...
switch ($d)
    {
    case 4:     $dlzka=11; $dlzka4=4; break;
    case 5:     $dlzka=9; break;
    case 6:     $dlzka=8; break;
    }
